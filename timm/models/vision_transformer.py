...
class Block(nn.Module):
    ...
    def forward(self, x, features_to_be_reused=None, relations_to_be_reused=None):
        ...
        if features_to_be_reused is not None:
            ...
            # print(feature_temp.size())

            # if self.reuse_with_cls_token:
            # feature_temp = torch.cat((features_to_be_reused[:, 0].view(B, 1, 48), feature_temp), dim=1)
            feature_temp = torch.cat((torch.zeros(B, 1, 48).cuda(), feature_temp), dim=1)
            # feature_temp = torch.cat((torch.zeros(B, 1, 48), feature_temp), dim=1)
            # print(feature_temp.shape)
            # x = x + feature_temp
            ...
            # else:
            #     # x[:, 1:] = x[:, 1:] + feature_temp
            #     x[:, 1:] = torch.cat((x[:, 1:], feature_temp), dim=2)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        if patch_size[0] == 16 or patch_size[0] == 32:
            ...
            num_patches = 100
            self.num_patches = num_patches
            self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=23, stride=23, padding=3)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                feature_reuse=feature_reuse, relation_reuse=relation_reuse)
        ...
        if relation_reuse:
            ...
            if self.patch_embed.num_patches == 100:
    ...
    def forward_features(self, x, features_to_be_reused_list, relations_to_be_reused_list):
        ...
        if relations_to_be_reused_list is not None:
            ...
            split_index = int(relation_temp.size(0) / 4)
            relation_temp = torch.cat(
                (
                    self.relation_reuse_upsample(relation_temp[:split_index * 1]),
                    self.relation_reuse_upsample(relation_temp[split_index * 1:split_index * 2]),
                    self.relation_reuse_upsample(relation_temp[split_index * 2:split_index * 3]),
                    self.relation_reuse_upsample(relation_temp[split_index * 3:]),
                ), 0
            )
            ...
            split_index = int(relation_temp.size(0) / 8)
            relation_temp = torch.cat(
                (
                    self.relation_reuse_upsample(relation_temp[:split_index * 1]),
                    self.relation_reuse_upsample(relation_temp[split_index * 1:split_index * 2]),
                    self.relation_reuse_upsample(relation_temp[split_index * 2:split_index * 3]),
                    self.relation_reuse_upsample(relation_temp[split_index * 3:split_index * 4]),
                    self.relation_reuse_upsample(relation_temp[split_index * 4:split_index * 5]),
                    self.relation_reuse_upsample(relation_temp[split_index * 5:split_index * 6]),
                    self.relation_reuse_upsample(relation_temp[split_index * 6:split_index * 7]),
                    self.relation_reuse_upsample(relation_temp[split_index * 7:]),
                ), 0
            )
            relation_temp = self.adaptive_avgpool(relation_temp)
